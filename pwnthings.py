...
if sys.version_info[0]==3:
    try:
        ...
        if os=="posix":
            ...
            execute('sudo apt install apktool')
        elif os=="nt":
            print(f'[ WINDOWS ]')
...
{Fore.RED}[X]{Fore.WHITE} Unpublished For Beta Testers Only
    ...
    #random_proxy
    def random_proxy(type):
        api=f'http://pubproxy.com/api/proxy?limit=5&last_check=60&level=anonymous&type={type}'
        api_json=requests.get(api).json()
        api_dump=json.dumps(api_json)
        proxy_list=json.loads(api_dump)
        for proxy in proxy_list['data']:
            print(f'''
{Fore.GREEN}[+] IP:Port :{Fore.WHITE} {proxy['ipPort']}
{Fore.GREEN}[+] Proxy Level :{Fore.WHITE} {proxy['proxy_level']}
{Fore.GREEN}[+] Type :{Fore.WHITE} {proxy['type']}
{Fore.GREEN}[+] Country :{Fore.WHITE} {proxy['country']}
{Fore.GREEN}[+] Last Checked :{Fore.WHITE} {proxy['last_checked']}
{Fore.GREEN}[+] Connecting Speed in Sec. :{Fore.WHITE} {proxy['speed']}
...
con_client('{ip}',{port})
    ...
    try:
        ...
        parser.add_argument("-apktoold", "--apktoold", help="to Decompile a apk using apktool  example: tool.py -apktoold test.apk ", type=str)
        parser.add_argument("-apktoolr", "--apktoolr", help="to Recompile a apk source using apktool  example: tool.py -apktoolr test/ ", type=str)
        parser.add_argument("-randomproxy", "--randomproxy", help="Generate 5 Random Proxy in each request  example: tool.py -randomproxy http ", type=str)
        ...
        elif args.apktoold is not None:
            logo()
            execute(f'apktool d {args.apktoold}')
        elif args.apktoolr is not None:
            logo()
            execute(f'apktool b {args.apktoolr} -o {args.apktoolr}.apk')
        elif args.randomproxy is not None:
            logo()
            random_proxy(args.randomproxy)
